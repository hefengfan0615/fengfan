...
inline int futility_depth(Value eval, Value beta) {
    ...
    //      depth = 9 + int(0.5 + 6 / int(1 + pow(abs(eval) + abs(beta), 3) / 50'000'000'000))
...
// ============================================================================
// Duffish-style Aggressiveness.
// Ported from the Duffish xiangqi engine (https://github.com/lxsgx23/Duffish).
// These helpers quantify attacking pressure against the enemy king, guard
// (advisor/bishop) damage, sacrifice quality and palace threats. The results
// shape the static evaluation (see apply_duffish_eval_shaping) and protect
// attacking moves during the search.
// ============================================================================

// Rank of a square from the point of view of color c (0 = its own back rank).
inline int xiangqi_relative_rank(Color c, Square s) {
...
// Palace (九宫) of color c: the three central files of its own first three ranks.
Bitboard xiangqi_palace_zone(const Position& pos, Color c) {
    ...
    for (int f = std::max(0, kFile - 1); f <= std::min(int(FILE_NB) - 1, kFile + 1); ++f)
        for (int r = 0; r < int(RANK_NB); ++r)
            if (xiangqi_relative_rank(c, make_square(File(f), Rank(r))) <= 2)
...
// Root pieces that can take part in an attack: rooks, cannons, knights, pawns.
...
bool xiangqi_attack_shape(const Position& pos, Color root) {
    ...
    while (pieces)
    {
        ...
        if (pt == PAWN && xiangqi_relative_rank(root, s) >= 5
...
int xiangqi_sacrifice_quality(const Position& pos, Color root, int materialLoss, int pressure) {
    ...
    while (guards)
    {
        Square g = pop_lsb(guards);
        if (pos.attackers_to(g) & attackers)
            guardThreats++;
    }
...
// The heart of the Aggressiveness feature: shape the (NNUE) static evaluation
// from the root side's perspective. When the Aggressiveness option is above
// 100 the engine rewards attack pressure, palace threats, sacrifice
// compensation and damage to the opponent's guard structure.
Value apply_duffish_eval_shaping(const Search::Worker& w, const Position& pos, Value v) {
    int evalDecay = w.cachedEvalDecay;
    ...
    int aggressiveness = w.cachedAggressiveness;
    if (aggressiveness == 100 && w.cachedSacBonus == 0 && w.cachedSacDetect == 0)
        return v;

    Color root     = w.rootColor < COLOR_NB ? w.rootColor : pos.side_to_move();
    ...
    int startBalance =
      w.rootColor < COLOR_NB ? w.rootMaterialBalance : currentBalance;
    ...
    if (sacBonus > 0 && rootMaterialLoss > 0)
    {
        ...
        else
        {
            ...
            bonus += w.cachedDynamicComp * std::min(quality, 360) / 760;
    ...
    {
        Color them = ~root;
        int   advisorBreakBonus = w.cachedAdvisorBreakBonus;
        int   bishopBreakBonus  = w.cachedBishopBreakBonus;
    ...
    rootEval = std::clamp(rootEval, int(VALUE_MATED_IN_MAX_PLY) + 1, int(VALUE_MATE_IN_MAX_PLY) - 1);
    return Value(rootSign * rootEval);
}

// Root side's material loss in the current node, relative to the start of the search.
int xiangqi_root_material_loss(const Search::Worker& w, int materialBalance) {
    if (w.rootColor >= COLOR_NB)
        return 0;
    return std::max(0, w.rootMaterialBalance - materialBalance);
}

// Material balance of the position after 'move' (only captures change it).
int xiangqi_next_material_balance(const Position& pos,
                                  const Search::Worker& w,
                                  int materialBalance,
                                  Move move) {
    if (w.rootColor >= COLOR_NB || !pos.capture(move))
    ...
    int capturedValue = int(PieceValue[captured]);
    return color_of(captured) == w.rootColor ? materialBalance - capturedValue
                                             : materialBalance + capturedValue;
}

int xiangqi_position_sacrifice_trace(const Position& pos,
                                     const Search::Worker& w,
                                     int materialBalance) {
    if (w.rootColor >= COLOR_NB)
        return 0;

    int rootLoss = xiangqi_root_material_loss(w, materialBalance);
    if (rootLoss <= 0)
        return 0;

    int pressure    = xiangqi_attack_pressure(pos, w.rootColor);
    int guardDamage = xiangqi_guard_damage(pos, w.rootColor);
    int quality     = xiangqi_sacrifice_quality(pos, w.rootColor, rootLoss, pressure);
...
// Running sacrifice trace propagated down the tree.
int xiangqi_next_sacrifice_trace(const Position& pos,
                                 const Search::Worker& w,
                                 Stack* ss,
                                 Move move,
                                 bool givesCheck,
                                 int nextMaterialBalance) {
    if (w.rootColor >= COLOR_NB)
        return 0;

    int trace = std::max(0, ss->sacrificeTrace - 6);
    trace     = std::max(trace, xiangqi_position_sacrifice_trace(pos, w, nextMaterialBalance));

    Color root  = w.rootColor;
    int   oldLoss = xiangqi_root_material_loss(w, ss->materialDiff);
    int   newLoss = xiangqi_root_material_loss(w, nextMaterialBalance);
    if (newLoss > 0)
    {
        ...
        if (quality >= required)
            trace += std::min(70, (quality - required) / 3 + w.cachedDynamicComp / 3);
        else if (quality < required / 2)
    ...
    if (pos.side_to_move() == root)
    {
        ...
        PieceType mpt = type_of(moved);
        bool attackerType = mpt == ROOK || mpt == CANNON || mpt == KNIGHT || mpt == PAWN;

        if (attackerType && (givesCheck || pos.capture(move)))
        {
            if (pos.capture(move) && !pos.see_ge(move, -120))
                trace += 70;
            else if (!pos.capture(move) && xiangqi_near_enemy_palace(pos, root, move.to_sq()))
                trace += 35;
        }
    }
